async function draw() {
  ...
  const cidades = Array.from(d3.group(links.filter(d => d.source != ""), d => d.source).keys());
  ...
  link.append("path")
    ...
    .on("click", (event, element) => {
      const uf = d3.select("#filter-uf").property("value");
      const cidades = d3.select("#filter-cidades").property("value");
      const product = d3.select("#filter-product").property("value");
      if (checkFilterApplied(uf, product)) {
        if (element.uf === uf || element.funcao === product) {
