async function draw() {
  ...
  const links = await d3.csv("../sankey_android/dados.csv");
  links.map(l => {
    l.value = +l.value;
    l.count = +l.count;
  });
  ...
  const colorMunicipio = d3.scaleOrdinal()
    .domain([
      ...
      "não tem correspondência em inglês",
  ...
  link.append("path")
    ...
    .on("click", (event, element) => {
        var corda = this;
        d3.select("#link" + element.index)
          .each(function() {
           var cordaR = this;
           d3.select("#link" + element.index).attr("stroke-opacity", function() {
            return (cordaR === corda) ? "0.3" : "1";
        });
      });
});
